...
ALERTED = os.path.join(STATE_DIR, "alerted.json")     # alert dedupe, shared with intraday
ACCUM = os.path.join(STATE_DIR, "candidates_today.json")  # intraday-pulled calls for paper ingest
...
def run(candidates, today):
    ...
    # 2) merge the intraday accumulator (message-level dedupe hides those
    #    calls from this run's pull, but the paper book still needs them)
    have = {f"{c['ticker']}:{c['date']}" for c in candidates}
    if os.path.exists(ACCUM):
        for c in json.load(open(ACCUM)):
            k = f"{c['ticker']}:{c['date']}"
            if k not in have: candidates.append(c); have.add(k)
    # 3) ingest: confluence tier alert (unless already alerted intraday) + paper book
    import confluence
    confluence.append_candidates(candidates)
    ...
    for c in candidates:
        ...
        if key not in alerted:
            ...
            xok, xdetail = confluence.internals(c["date"])
            xs = "ok" if xok else ("NODATA" if xok is None else "RISK")
            print(f"scored {c['ticker']} {c['date']}: {score} [{flags}] X={xs}({xdetail}) fmt={c.get('fmt','?')} size={c.get('size_pct','?')}%")
            if tier:
                alerts.append(f"{c['ticker']} | {tier} | ENTRY {c['entry']} | SSL {c['sl']}")
        ...
        if pos:
            ...
            pos["tier"] = a.get("tier"); pos["flags"] = a.get("flags")
            for e in advance(pos, today): pass  # catch up any same-window action
            state["positions"].append(pos)
    # 4) finalize provisional B for today's intraday alerts; DEMOTED note on change
    for key, a in alerted.items():
        if a.get("date") != today or a.get("final"): continue
        tier2, flags2, score2 = confluence.tier_for(dict(ticker=a["ticker"], date=a["date"]))
        if a.get("tier") and (tier2 or "skip") != a["tier"]:
            alerts.append(f"{a['ticker']} | DEMOTED {a['tier']}->{tier2 or 'skip'} | later call(s) same day")
            print(f"finalized {key}: {a['tier']} -> {tier2 or 'skip'} [{flags2}]")
        a.update(tier=tier2, flags=flags2, score=score2, final=True)
    json.dump(alerted, open(ALERTED, "w"), indent=2)
    if os.path.exists(ACCUM): os.remove(ACCUM)
    save_state(state)
    sc = scorecard(state)
